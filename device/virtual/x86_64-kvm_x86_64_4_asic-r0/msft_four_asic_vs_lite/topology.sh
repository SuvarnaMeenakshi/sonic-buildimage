...
start () {
    ...
    # asic0 eth1 --- asic0 eth1
    # asic0 eth2 --- asic0 eth2
    # asic1 eth3 --- asic1 eth1
    # asic1 eth4 --- asic1 eth2
    ...
    for BACKEND in `seq $FIRST_BACKEND_ASIC $LAST_BACKEND_ASIC`; do
        for FRONTEND in `seq $FIRST_FRONTEND_ASIC $LAST_FRONTEND_ASIC`; do
            FRONT_NAME="eth$(($(($BACKEND - $FIRST_BACKEND_ASIC)) + 3))"
            BACK_NAME="eth$(($FRONTEND + 1))"
